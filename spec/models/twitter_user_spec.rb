...
RSpec.describe TwitterUser, type: :model do
  ...
  describe '::find_or_create_from_auth_hash' do
    ...
    context '新規ユーザの場合' do
      ...
      it 'User#usernameに引数のキー[:info][:name]の値をセットして保存すること' do
      ...
      it 'TwitterUser#nicknameに引数のキー[:info][:nickname]の値をセットして保存すること' do
        twitter_user = TwitterUser.find_or_create_from_auth_hash(auth_hash)
        found_twitter_user = TwitterUser.find(twitter_user.id)
        expect(found_twitter_user.nickname).to eq auth_hash[:info][:nickname]
      end
